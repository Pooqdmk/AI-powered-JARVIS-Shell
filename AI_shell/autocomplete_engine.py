...
def load_bart_model(model_dir: Optional[str] = MODEL_DIR):
    try:
        if model_dir and os.path.isdir(model_dir):
            ...
            print(f"🔍 Loaded BART from local dir: {model_dir}")
        else:
            ...
            print(f"🌐 Loaded BART from HF: {HF_REPO}/{SUBFOLDER}")
    except Exception as e:
        print(f"⚠️ Failed loading fine-tuned model: {e}\nFalling back to {BASE_MODEL}")
...
async def suggest(text: str, limit: int = MAX_SUGGESTIONS) -> List[str]:
    ...
    try:
        ...
        # This allows proper error logging in the TUI
        raise Exception(f"BART model generation failed: {e}") from e
